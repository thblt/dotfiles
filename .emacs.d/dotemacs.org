...
An extra function to quickly commit changes to all open files in current project:
  ...
  (defun thblt-magit-commit-all ()
    ...
    (let ((unstaged-files (magit-unstaged-files))
      ...
      (if (= 0 (length unstaged-files))
        ...
        (with-current-buffer file-list-buffer
        ...
        (setq commit-msg (read-input "Commit message: "))
        (if (= 0 (length commit-msg))
            (message "No commit message, aborting.")
          (magit-commit (list "-m" commit-msg)))
        (delete-window (get-buffer-window file-list-buffer))
        (bury-buffer file-list-buffer))))

  (global-set-key (kbd "C-x g") 'thblt-magit-commit-all)
#+end_src
